...
contract Domains is ERC721URIStorage {
    ...
    modifier isOwnerOf(string calldata domainName) {
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        if (len == 3) {
            ...
            return 5 * 10**17; //5 matic 5000.. 18 decimal places
        } else if (len == 4) {
            return 3 * 10**17; // 3 matic
        } else {
            return 10**17; // 1 matic
        }
    }

    constructor(string memory _tld)
        payable
        ERC721("Polygon Domain Name Service", "NS")
    {
        tld = _tld;
        console.log("This is my domain contract (printed from constructor)");
    ...
    function register(string calldata name) public payable {
        ...
        require(msg.value > _price, "Not enough fees paid!");
        ...
        console.log(
            "Registering %s.%s on contract with tokein id %s",
            name,
            tld,
            newRecordId
        );
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                name,
                '", "description": A domain on SB name service, "image": "data:image/svg+xml;base64,',
        ...
        console.log("Final Toke URI: ", finalTokenUri);
        ...
        _tokenIds.increment();
        console.log("%s has registered domain name!", msg.sender);
    }
    ...
    function getAllNames() public view returns (string[] memory) {
        ...
        for (uint256 i = 0; i < _tokenIds.current(); i++) {
            allNames[i] = names[i];
            console.log("name for token id %s is %s", i, allNames[i]);
        }
        return allNames;
    }
}
